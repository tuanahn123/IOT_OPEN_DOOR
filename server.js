...
app.use((req, res, next) => {
  console.log(
    `Admin Log: ${req.method} ${req.originalUrl} - ${new Date().toISOString()}`
  );
  next();
});
...
app.post("/users", async (req, res) => {
  ...
  try {
    ...
    const existingUser = await user.findOne({
      $or: [{ pin_code: pin_code || null }, { rfid_code: rfid_code || null }],
    });
...
app.post("/open-door", async (req, res) => {
  ...
  try {
    ...
    const foundUser = await user.findOne(
      pin_code ? { pin_code: pin_code } : { rfid_code: rfid_code }
    );
...
app.delete("/users/:id", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Người dùng và lịch sử mở cửa đã được xóa thành công",
    });
  } catch (error) {
...
app.post("/open-door-manually", async (req, res) => {
  try {
    ...
    if (response.ok) {
      ...
      const code = "1111";
      const adminUser = await user.findOne({ pin_code: code });
      ...
      if (adminUser) {
        ...
        return res
          .status(401)
          .json({ message: "Mã RFID của admin không hợp lệ" }); // Phản hồi không thành công
      }
    } else {
      console.log("Failed to open door. Response not OK.");
      return res
        .status(500)
        .json({ success: false, message: "Không thể mở cửa" });
    }
  } catch (error) {
    console.error("Lỗi:", error);
    return res
      .status(500)
      .json({ success: false, message: "Lỗi kết nối đến ESP32" });
  }
});
